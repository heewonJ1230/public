...
### 2025-11-03 - 스냅샷 업로드 예전버전 가지고 와서 업데이트 해달라고 함
- 하객 스냅샷 업로드 안되던 문제 다시 도전
- 모바일 판별 로직을 보강해 iPad 데스크톱 모드에서도 하객 업로드 버튼이 정상 노출되도록 수정.
- 하객 업로드 상태 메시지를 한국어 이모지 버전으로 교체하고, 업로드 성공 시 `Memorized Memories` 섹션만 자동 새로고침.
- `Memorized Memories 🔃` 수동 새로고침 버튼을 추가하고 로딩 상태를 표시하도록 처리.
- 축하 메시지 등록 함수를 `async`로 전환해, 작성 직후 메시지 리스트가 자동으로 새로고침되도록 개선.
